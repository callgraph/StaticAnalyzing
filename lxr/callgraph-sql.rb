...
#               pre_path=path+"/"
                if path.index(".c") || path.index(".S") || path.index(".h")     #modify algorithm from 20150610 by jdi -- exclude sline about .c||.S||.h
                        pre_path=path
                else
                        pre_path=path+"/"
                end
...
####print include nodes color ,URL=callgraph-link,each node with one special color
		for j in 1..($number_path)
			for i in $number_in[j-1]..($number_in[j]-1)
				wfile.print %Q{"#{$sline[i]}"}+"[tooltip="+%Q{"#{$sline[i]} #{$sline_time[i]},#{$sline_power[i]}"}+"]"+"[label="+%Q{"#{$sline[i]}"}+","
