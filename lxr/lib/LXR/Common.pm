...
our @EXPORT = qw(
	...
	&warning &fatal
	&fflush
	&urlargs &nonvarargs &fileref &diffref &idref &idrefcall &idreflist &idreftaint &idrefvulner &idrefsystrace &incref &idrefdiffe &idrefenergy &idrefbinder &idrefinst        &idrefimport &idrefdcgAnalyser
	&httpinit &httpclean
);
# our @EXPORT_OK = qw(
#   &abortall 
...
sub idrefsystrace {
        my ($desc, $css, $id, @args) = @_;
        return ("<a class='$css' href=\"$config->{virtroot}/systrace-perl"
                  . &urlargs    ( ($id ? "_i=$id" : "")
                                                , &nonvarargs()
                                                , @args
                                                )
...
sub idrefdiffe {
        my ($desc, $css, $id, @args) = @_;
        return ("<a class='$css' href=\"$config->{virtroot}/diffe"
                  . &urlargs    ( ($id ? "_i=$id" : "")
                                                , &nonvarargs()
                                                , @args
                                                )
...
sub idrefenergy {
        my ($desc, $css, $id, @args) = @_;
        return ("<a class='$css' href=\"$config->{virtroot}/energy"
                  . &urlargs    ( ($id ? "_i=$id" : "")
                                                , &nonvarargs()
                                                , @args
                                                )
...
sub idrefbinder {
        my ($desc, $css, $id, @args) = @_;
        return ("<a class='$css' href=\"$config->{virtroot}/binder"
                  . &urlargs    ( ($id ? "_i=$id" : "")
                                                , &nonvarargs()
                                                , @args
                                                )
...
sub idrefinst {
        my ($desc, $css, $id, @args) = @_;
        return ("<a class='$css' href=\"$config->{virtroot}/inst"
                  . &urlargs    ( ($id ? "_i=$id" : "")
                                                , &nonvarargs()
                                                , @args
                                                )
...
sub idrefimport {
        my ($desc, $css, $id, @args) = @_;
        return ("<a class='$css' href=\"$config->{virtroot}/import"
                  . &urlargs    ( ($id ? "_i=$id" : "")
                                                , &nonvarargs()
                                                , @args
                                                )
...
####### by aquan
sub idrefdcgAnalyser {
        my ($desc, $css, $id, @args) = @_;
        return ("<a class='$css' href=\"$config->{virtroot}/dcgAnalyser"
                  . &urlargs    ( ($id ? "_i=$id" : "")
                                                , &nonvarargs()
                                                , @args
                                                )
...
#	if(!($ENV{'QUERY_STRING'}=~'path0')){
               ...
               # $ENV{'QUERY_STRINGTID'}='testcase';
#end-lixue
#=head
                my @value=split /&/,$ENV{'QUERY_STRING'};
                $ENV{'QUERY_STRINGSIDNUM'}=length($value[0]);
                $ENV{'QUERY_STRINGSID1'}=substr($value[0],2);
		$ENV{'QUERY_STRINGSID'}='ucore' if substr($value[0],2) eq 'ucore';
                $ENV{'QUERY_STRINGSID'}='linux-3.5.4' if substr($value[0],2) eq 'linux-3.5.4';
                $ENV{'QUERY_STRINGSID'}='linux-3.8.13' if substr($value[0],2) eq 'linux-3.8.13';
                $ENV{'QUERY_STRINGSID'}='linux-2.6.39.4' if substr($value[0],2) eq 'linux-2.6.39.4';
                $ENV{'QUERY_STRINGSID'}='Android-4.4.3' if substr($value[0],2) eq 'Android-4.4.3';
                $ENV{'QUERY_STRINGSID'}='linux-3.19.1' if substr($value[0],2) eq 'linux-3.19.1';###added by aoquan
...
# $ENV{'QUERY_STRINGSID'}=~s/[\r\n]//;
                $ENV{'QUERY_STRINGPID'}='arm-pandaboard' if substr($value[2],2) eq 'arm-pandaboard';
               $ENV{'QUERY_STRINGPID'}='arm-Raspberrypi' if substr($value[2],2) eq 'arm-Raspberrypi';
#               $ENV{'QUERY_STRINGPID'}='arm-galaxy-nexus' if substr($value[2],2) eq 'arm-galaxy-nexus';
               $ENV{'QUERY_STRINGPID'}='x86_64' if substr($value[2],2) eq 'x86_64';
                $ENV{'QUERY_STRINGPID'}='x86_32' if substr($value[2],2) eq 'x86_32';
                $ENV{'QUERY_STRINGPID'}='arm-Nexus5' if substr($value[2],2) eq 'arm-Nexus5';
...
#lixue-add-20141230
               $ENV{'QUERY_STRINGTID'}='testcase1' if substr($value[3],2) eq 'testcase1';
                $ENV{'QUERY_STRINGTID'}='testcase2' if substr($value[3],2) eq 'testcase2';
                $ENV{'QUERY_STRINGTID'}='testcase3' if substr($value[3],2) eq 'testcase3';
#end-lixue
