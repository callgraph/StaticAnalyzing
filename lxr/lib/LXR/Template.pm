...
#20131017begin
                ...
                my $s = $HTTP->{'param'}{'_string'};
                $ret = $config->sourcerootname . ' Call Graph'
                                . ($s ? ": $s" : '');
        }
        elsif ($who eq 'watchlist') {
                my $s = $HTTP->{'param'}{'_string'};
                $ret = $config->sourcerootname . ' Function List'
                                . ($s ? ": $s" : '');
        }
        elsif ($who eq 'taintrace') {
                my $s = $HTTP->{'param'}{'_string'};
                $ret = $config->sourcerootname . ' Taint Trace'
                                . ($s ? ": $s" : '');
        }
        elsif ($who eq 'vulnermap') {
                my $s = $HTTP->{'param'}{'_string'};
                $ret = $config->sourcerootname . ' Vulnerability Map'
                                . ($s ? ": $s" : '');
        }
	elsif ($who eq 'systrace-perl') {
               my $s = $HTTP->{'param'}{'_string'};
               $ret = $config->sourcerootname . 'Systrace Map'
                                 . ($s ? ": $s" : '');
       }
       elsif ($who eq 'diffe') {
                 my $s = $HTTP->{'param'}{'_string'};
                 $ret = $config->sourcerootname . ' Diffe'
                                 . ($s ? ": $s" : '');
         }
       elsif ($who eq 'energy') {
                 my $s = $HTTP->{'param'}{'_string'};
                 $ret = $config->sourcerootname . ' Energy'
                                 . ($s ? ": $s" : '');
         }
       elsif ($who eq 'binder') {
                 my $s = $HTTP->{'param'}{'_string'};
                 $ret = $config->sourcerootname . ' Binder'
                                 . ($s ? ": $s" : '');
         }
       elsif ($who eq 'inst') {
                 my $s = $HTTP->{'param'}{'_string'};
                 $ret = $config->sourcerootname . ' Instruction'
                                 . ($s ? ": $s" : '');
         }
       elsif ($who eq 'import') {
                 my $s = $HTTP->{'param'}{'_string'};
                 $ret = $config->sourcerootname . ' Tracing Data Import'
                                 . ($s ? ": $s" : '');
         }
	##### by aquan
       elsif ($who eq 'dcgAnalyser') {
                 my $s = $HTTP->{'param'}{'_string'};
                 $ret = $config->sourcerootname . ' dcgAnalyser'
                                 . ($s ? ": $s" : '');
         }
        ###############	
...
#20131017begin
		...
		$modeselect = "selected";
                ...
                $modelink = idrefcall($modename, "modes", "");
                $modecss  = "modes";
                $modeaction = "$config->{virtroot}/call";
                $modeoff  = "";
		$modeselect = "";
        }
        push(@mlist,    { 'name' => $modename
       ...
       $modename = "Function List";
		...
		$modeselect = "selected";
                ...
                $modelink = idreflist($modename, "modes", "");
                $modecss  = "modes";
                $modeaction = "$config->{virtroot}/watchlist";
                $modeoff  = "";
		$modeselect = "";
        }
        push(@mlist,    { 'name' => $modename
		...
		$modeselect = "selected";
                ...
                $modelink = idreftaint($modename, "modes", "");
                $modecss  = "modes";
                $modeaction = "$config->{virtroot}/taintrace";
                $modeoff  = "";
		$modeselect = "";
        }
        push(@mlist,    { 'name' => $modename
      ...
      $modename = "Vulnerability Map";
		...
		$modeselect = "selected";
                ...
                $modelink = idrefvulner($modename, "modes", "");
                $modecss  = "modes";
                $modeaction = "$config->{virtroot}/vulnermap";
                $modeoff  = "";
		$modeselect = "";
        }
        push(@mlist,    { 'name' => $modename
	...
	$modename = "Systrace Map";
         if ($who eq 'systrace-perl')
         {       $modelink = "<span class='modes-sel'>$modename</span>";
                 $modecss  = "modes-sel";
                 $modeaction= "";
                 $modeoff  = "disabled";
		$modeselect = "selected";
                 ...
                 $modelink = idrefcall($modename, "modes", "");
                 $modecss  = "modes";
                 $modeaction = "$config->{virtroot}/systrace-perl";
                 $modeoff  = "";
		 $modeselect= "";
         }
         push(@mlist,    { 'name' => $modename
       ...
       $modename = "Diffe";
		...
		$modeselect= "selected";
                 ...
                 $modelink = idrefdiffe($modename, "modes", "");
                 $modecss  = "modes";
                 $modeaction = "$config->{virtroot}/diffe";
                 $modeoff  = "";
		 $modeselect= "";
         }
         push(@mlist,    { 'name' => $modename
       ...
       $modename = "Energy";
		 ...
		 $modeselect = "selected";
                 ...
                 $modelink = idrefenergy($modename, "modes", "");
                 $modecss  = "modes";
                 $modeaction = "$config->{virtroot}/energy";
                 $modeoff  = "";
		 $modeselect= "";
         }
         push(@mlist,    { 'name' => $modename
	...
	$modename = "Binder";
		 ...
		 $modeselect ="selected";
                 ...
                 $modelink = idrefbinder($modename, "modes", "");
                 $modecss  = "modes";
                 $modeaction = "$config->{virtroot}/binder";
                 $modeoff  = "";
		 $modeselect= "";
         }
         push(@mlist,    { 'name' => $modename
       ...
       $modename = "Instruction";
		 ...
		 $modeselect ="selected";
                 ...
                 $modelink = idrefinst($modename, "modes", "");
                 $modecss  = "modes";
                 $modeaction = "$config->{virtroot}/inst";
                 $modeoff  = "";
		$modeselect ="";
         }
         push(@mlist,    { 'name' => $modename
       ...
       $modename = "Tracing Data Import";
		 ...
		 $modeselect ="selected";
                 ...
                 $modelink = idrefimport($modename, "modes", "");
                 $modecss  = "modes";
                 $modeaction = "$config->{virtroot}/import";
                 $modeoff  = "";
		 $modeselect="";
         }
         push(@mlist,    { 'name' => $modename
...
######## by aquan
       $modename = "dcgAnalyser";
         if ($who eq 'dcgAnalyser')
         {       $modelink = "<span class='modes-sel'>$modename</span>";
                 $modecss  = "modes-sel";
                 $modeaction= "";
                 $modeoff  = "disabled";
		 $modeselect ="selected";
                 ...
                 $modelink = idrefdcgAnalyser($modename, "modes", "");
                 $modecss  = "modes";
                 $modeaction = "$config->{virtroot}/dcgAnalyser";
                 $modeoff  = "";
		 $modeselect="";
         }
         push(@mlist,    { 'name' => $modename
...
#20131017begin
                                ...
                                $vallink =
                                    "<a class=\"varlink\" href=\"$config->{virtroot}/call"
                                  . &urlargs("$var=$val", "_string=" . $HTTP->{'param'}{'_string'})
                                  . "\">$val</a>";
                        }
                        elsif ($who eq 'watchlist') {
                                $vallink =
                                    "<a class=\"varlink\" href=\"$config->{virtroot}/watchlist"
                                  . &urlargs("$var=$val", "_string=" . $HTTP->{'param'}{'_string'})
                                  . "\">$val</a>";
                        }
                      elsif ($who eq 'taintrace') {
                                $vallink =
                                    "<a class=\"varlink\" href=\"$config->{virtroot}/taintrace"
                                  . &urlargs("$var=$val", "_string=" . $HTTP->{'param'}{'_string'})
                                  . "\">$val</a>";
                        }
                      elsif ($who eq 'vulnermap') {
                                $vallink =
                                    "<a class=\"varlink\" href=\"$config->{virtroot}/vulnermap"
                                  . &urlargs("$var=$val", "_string=" . $HTTP->{'param'}{'_string'})
                                  . "\">$val</a>";
                        }
        		 elsif ($who eq 'systrace-perl') {
                                 $vallink =
                                    "<a class=\"varlink\" href=\"$config->{virtroot}/systrace-perl"
                                   . &urlargs("$var=$val", "_string=" . $HTTP->{'param'}{'_string'})
                                   . "\">$val</a>";
                         }
                       elsif ($who eq 'diffe') {
                                 $vallink =
                                     "<a class=\"varlink\" href=\"$config->{virtroot}/diffe"
                                   . &urlargs("$var=$val", "_string=" . $HTTP->{'param'}{'_string'})
                                   . "\">$val</a>";
                         }
                       elsif ($who eq 'energy') {
                                $vallink =
                                     "<a class=\"varlink\" href=\"$config->{virtroot}/energy"
                                   . &urlargs("$var=$val", "_string=" . $HTTP->{'param'}{'_string'})
                                   . "\">$val</a>";
                        }
                       elsif ($who eq 'binder') {
                                $vallink =
                                     "<a class=\"varlink\" href=\"$config->{virtroot}/binder"
                                   . &urlargs("$var=$val", "_string=" . $HTTP->{'param'}{'_string'})
                                   . "\">$val</a>";
                         }
                       elsif ($who eq 'inst') {
                                 $vallink =
                                     "<a class=\"varlink\" href=\"$config->{virtroot}/inst"
                                   . &urlargs("$var=$val", "_string=" . $HTTP->{'param'}{'_string'})
                                   . "\">$val</a>";
                         }
                       elsif ($who eq 'import') {
                                 $vallink =
                                     "<a class=\"varlink\" href=\"$config->{virtroot}/import"
                                   . &urlargs("$var=$val", "_string=" . $HTTP->{'param'}{'_string'})
                                   . "\">$val</a>";
                         }
		########### by aquan
                                 ...
                                 $vallink =
                                     "<a class=\"varlink\" href=\"$config->{virtroot}/dcgAnalyser"
                                   . &urlargs("$var=$val", "_string=" . $HTTP->{'param'}{'_string'})
                                   . "\">$val</a>";
                         }
		#####################
...
#20131017begin
                ...
                $action =       "href=\"$config->{virtroot}/call"
                                                . &urlargs(&nonvarargs())
                                                . "\">";
        }
       elsif ($who eq 'watchlist') {
                $action =       "href=\"$config->{virtroot}/watchlist"
                                                . &urlargs(&nonvarargs())
                                                . "\">";
        }
       elsif ($who eq 'taintrace') {
                $action =       "href=\"$config->{virtroot}/taintrace"
                                                . &urlargs(&nonvarargs())
                                                . "\">";
        }

      elsif ($who eq 'vulnermap') {
                $action =       "href=\"$config->{virtroot}/vulnermap"
                                                . &urlargs(&nonvarargs())
                                                . "\">";
        }
	elsif ($who eq 'systrace-perl') {
                 $action =       "href=\"$config->{virtroot}/systrace-perl"
                                                 . &urlargs(&nonvarargs())
                                                 . "\">";
         }
       elsif ($who eq 'diffe') {
                 $action =       "href=\"$config->{virtroot}/diffe"
                                                 . &urlargs(&nonvarargs())
                                                 . "\">";
         }
       elsif ($who eq 'energy') {
                 $action =       "href=\"$config->{virtroot}/energy"
                                                 . &urlargs(&nonvarargs())
                                                 . "\">";
        }
       elsif ($who eq 'binder') {
                 $action =       "href=\"$config->{virtroot}/binder"
                                                 . &urlargs(&nonvarargs())
                                                 . "\">";
         }
       elsif ($who eq 'inst') {
                 $action =       "href=\"$config->{virtroot}/inst"
                                                 . &urlargs(&nonvarargs())
                                                 . "\">";
         }
      elsif ($who eq 'import') {
                 $action =       "href=\"$config->{virtroot}/import"
                                                 . &urlargs(&nonvarargs())
                                                 . "\">";
         }
######### by aquan
      elsif ($who eq 'dcgAnalyser') {
                 $action =       "href=\"$config->{virtroot}/dcgAnalyser"
                                                 . &urlargs(&nonvarargs())
                                                 . "\">";
         }
#####################
	...
	foreach $var ($config->allvariables) {
		...
		$varex .= expandtemplate
					...
					,	( 'varname'  => sub { $config->vardescription($var) }
						...
						, 'varpid'   => sub {$ENV{'QUERY_STRINGPID'} }
						, 'varfid'   => sub {$ENV{'QUERY_STRINHFID'}}
						, 'varsid'   => sub {$ENV{'QUERY_STRINGSID'}}
						, 'vartid'   => sub {$ENV{'QUERY_STRINGTID'}}
...
sub makeheader {
	...
	print(
		...
		,	( # --for <head> section--
			...
			,	'pid'        => sub { $ENV{'QUERY_STRINGPID'}}
			,	'fid'        => sub { $ENV{'QUERY_STRINGFID'}}
			,	'sid'	     => sub { $ENV{'QUERY_STRINGSID'}}
			,	'tid'	     => sub { $ENV{'QUERY_STRINGTID'}}		
