...
$code_url="http://192.168.2.113/lxr/lxr-code/#{$version}/"
...
################added 20121217
#note_dir="/var/www/doxygen_kernel/#{$version}/html/files.html"# added 20121217  html文件所在路径
note_dir=inputpath[3]
$note_dir=""
$note_dir=note_dir
$d=""
...
def Readhtml.readfile(path_name,file_name)
        ...
        if indexname.index(".S")
          ...
          afile=File.new(file_name)
          while line=afile.gets
                postion1=line.index(">/mnt/freenas/DCG-RTL/source/#{$version}/#{indexname}<")
...
#			puts line.index("\<td class\=\"memname\">")
#			puts line.index("#{fuc_name}")
...
#	  puts ex_line[i]
	...
	#puts fuc_name
		...
		tt=Array.new
		tt=t_line.split("#")
        $ss="http://124.16.141.179/doxygen_kernel/"+$version+"/html/"+tt[0]
...
module Table
	...
	def Table.body2()
		...
		for i in 0..$c_inf1.size-1
			k=$c_inf3[i].split(" ")[2].to_i	#k中存放着函数定义位置的序号
                        if $doxygen_flag=="0"
		           $call_inf+="<tr><td>"+"#{i+1}"+"</td><td><a href=\""+"#{$code_url}"+"#{$c_inf3[i].split(" ")[0]}\#L#{$c_inf3[i].split(" ")[1].to_i}\""+">"+"#{$c_inf1[k]}"+"</a></td><td>"+"#{$c_inf2[k]}"+"</td><td>"+"#{$c_inf3[i].split(" ")[0]}"+"</td><td><a href=\""+"#{$c_inf4[k].split("&&")[1]}\""+">"+"#{$c_inf4[k].split("&&")[0]}"+"</a></td></tr>"  
                        else
	...
	def Table.sbody()
		for i in 0..$sd.size-1
                        if $doxygen_flag=="0"
		            $call_inf+="<tr><td>"+"#{i+1}"+"</td><td><a href=\""+"#{$code_url}"+"#{$sd[i].split(",,")[5]}\#L#{$sd[i].split(",,")[3].split(" ")[1].to_i}\""+">"+"#{$sd[i].split(",,")[0]}"+"</a></td><td>"+"#{$sd[i].split(",,")[2]}"+"</td><td>"+"#{$sd[i].split(",,")[-1]}"+"</td><td>"+"#{$sd[i].split(",,")[3].split(" ")[0]}"+"</td><td><a href=\""+"#{$sd[i].split(",,")[4].split("&&")[1]}\""+">"+"#{$sd[i].split(",,")[4].split("&&")[0]}"+"</a></td></tr>" 
...
#                             $call_inf+="<tr><td>"+"#{i+1}"+"</td><td><a href=\""+"#{$code_url}"+"#{$sd[i].split(",,")[5]}"+"\?v\=#{$ver_v}"+"\##{Loadm.change($sd[i].split(",,")[3].split(" ")[1].to_i)}\""+">"+"#{$sd[i].split(",,")[0]}"+"</a></td><td>"+"#{$sd[i].split(",,")[2]}"+"</td><td>"+"#{$sd[i].split(",,")[5]}"+"</td><td>"+"#{$sd[i].split(",,")[3].split(" ")[0]}"+"</td><td><a href=\""+"#{$note_url}"+"#{$sd[i].split(",,")[4].split("&&")[1]}\""+">"+"#{$sd[i].split(",,")[4].split("&&")[0]}"+"</a></td></tr>" 
	...
	def Table.stbody()
		for i in 0..$static.size-1
                      if $doxygen_flag=="0"
			$call_inf+="<tr><td>"+"#{$sd.size+1+i}"+"</td><td><a href=\""+"#{$code_url}"+"#{$static[i].split(",,")[5]}\#L#{$static[i].split(",,")[3].split(" ")[1].to_i}\""+">"+"#{$static[i].split(",,")[0]}"+"</a></td><td>"+"#{$static[i].split(",,")[2]}"+"</td><td>"+"0"+"</td><td>"+"#{$static[i].split(",,")[3].split(" ")[0]}"+"</td><td><a href=\""+"#{$static[i].split(",,")[4].split("&&")[1]}\""+">"+"#{$static[i].split(",,")[4].split("&&")[0]}"+"</a></td></tr>"  
                      else
	...
	def Table.dybody()
		for i in 0..$dynamic.size-1
                     if $doxygen_flag=="0"
			$call_inf+="<tr><td>"+"#{$sd.size+$static.size+1+i}"+"</td><td><a href=\""+"#{$code_url}"+"#{$dynamic[i].split(",,")[5]}\#L#{$dynamic[i].split(",,")[3].split("&&")[1].to_i}\""+">"+"#{$dynamic[i].split(",,")[0]}"+"</a></td><td>"+"0"+"</td><td>"+"#{$dynamic[i].split(",,")[2]}"+"</td><td>"+"#{$dynamic[i].split(",,")[3].split("&&")[0]}"+"</td><td><a href=\""+"#{$dynamic[i].split(",,")[4].split("&&")[1]}\""+">"+"#{$dynamic[i].split(",,")[4].split("&&")[0]}"+"</a></td></tr>"
                      else
		...
		temp_path3=""
			...
			if $f_vir=="real"
				...
				temp_path3=Virtual2real.only_file($c_inf3[i].split(" ")[0])
                        ...
                        sjoin=$c_inf1[i]+"\,\,X\,\,"+$c_inf2[i].to_s+"\,\,"+$c_inf3[i]+"\,\,"+$c_inf4[i]+"\,\,"+temp_path3
			...
			if $f_vir=="real"
				...
				temp_path3=Virtual2real.only_file($dc_inf3[i].split(" ")[0])
                        ...
                        djoin=$dc_inf1[i]+"\,\,XX\,\,"+$dc_inf2[i].to_s+"\,\,"+$dc_inf3[i]+"\,\,"+$dc_inf4[i]+"\,\,"+temp_path3
...
#file.puts("#{$static_time}  #{$dynamic_time}  #{end_time-begin_time}")
